...
#Specific storage
Ss = 1e-3 #specific storage coefficient [1/m]
k_c = 1.0 #rel perm
phi = 0.4  #porosity
theta_s = phi*s_s
theta_r =  phi*s_r
...
#Van Genuchten
sw_vG= lambda h,n: (1-np.heaviside(h,0))*((s_s - s_r)/((1 + np.abs(alpha_vG*h)**n)**(1-1/n))+s_r)+ np.heaviside(h,0)*(1) #sw, head in cms
theta_vG= lambda h,n: (1-np.heaviside(h,0))*((theta_s - theta_r)/((1 + np.abs(alpha_vG*h)**n)**(1-1/n))+theta_r)+ np.heaviside(h,0)*(theta_s+Ss*h) #sw, head in cms
kr_vG= lambda h,n: (1-np.heaviside(h,0))*((1-(np.abs(alpha_vG*h)**(n-1))/(1+np.abs(alpha_vG*h)**n)**(1-1/n))**2.0/((1 + np.abs(alpha_vG*h)**n)**((1-1/n)/2))) + np.heaviside(h,0)*(1+(Ss*h)**k_exp*k_c)  #kr, head in cms
...
#New
fig = plt.figure(figsize=(5,7.5) , dpi=100)
plt.plot(theta_vG(h,1),h,'-',color='k',alpha=0.2,label=r'$n=1$')
plt.plot(theta_vG(h,2),h,'-',color='k',alpha=0.4,label=r'$n=2$')
plt.plot(theta_vG(h,10),h,'-',color='k',alpha=0.6,label=r'$n=10$')
plt.plot(theta_vG(h,50),h,'-',color='k',alpha=0.8,label=r'$n=50$')
plt.plot(theta_vG(h,100),h,'-',color='k',alpha=1.0,label=r'$n=100$')
#manager = plt.get_current_fig_manager()
#manager.window.showMaximized()
plt.ylabel(r'$h$ [cm]')
plt.xlim([theta_r-0.01,theta_s+0.01])
plt.ylim([np.min(h),np.max(h)])
plt.xlabel(r'$\theta$')
plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
plt.legend()
